...
import { getProject, projects, type ProjectVideo as ProjectVideoData } from "@/lib/projects";
...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="case-page">
    ...
    {project.videos&&<section className="video-section"><p className="label">Watch it in action</p><div className="project-video-list">{project.videos.map((video)=><figure key={video.src}><ProjectVideo video={video} /><figcaption>{video.title}</figcaption></figure>)}</div></section>}
    <section className="case-overview"><div className="case-facts"><div><span>Team</span><strong>{project.team}</strong></div><div><span>Time</span><strong>{project.duration}</strong></div><div><span>Engine</span><strong>{project.engine}</strong></div></div><div className="case-prose"><h2>The brief</h2><p>{project.brief}</p><h2>How it developed</h2><p>{project.development}</p><h2>My contribution</h2><p>{project.role}</p></div></section>
